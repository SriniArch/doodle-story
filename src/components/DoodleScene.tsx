...
function DoodleProp({ object }: { object: ObjectFrame }) {
  ...
  if (name === "clock") {
    return (
      <g className="prop prop-clock" data-fx={fx ?? "none"} opacity={object.opacity} transform={`translate(${x} ${y})`}>
        <g className="clock-body">
          <circle r="19" />
          <path className="clock-hands" d="M0-12V0l9 5" />
          <path className="clock-bells" d="M-14-15l-7-7m35 7 7-7" />
        </g>
        {fx === "shake" ? <path className="clock-rings" d="M26-4c8 6 8 16 0 22M32-8c12 8 12 24 0 32" /> : null}
      </g>
    );
  }
  if (name === "bed") {
    return (
      <g className="prop prop-bed" opacity={object.opacity} transform={`translate(${x} ${y})`}>
  ...
  if (name === "coffee") {
    return (
      <g className="prop prop-coffee" data-fx={fx === "spill" ? "spill" : "none"} opacity={object.opacity} transform={`translate(${x} ${y})`}>
        ...
        {fx === "spill" ? <g className="coffee-miss"><path d="M-10 20c-2 7-1 14 2 18" /><path d="M-18 14c-3 8-2 14 1 20" /></g> : null}
      </g>
    );
  }
  if (name === "laptop") {
    return (
      <g className="prop prop-laptop" data-fx={fx ?? "idle"} opacity={object.opacity} transform={`translate(${x} ${y})`}>
  ...
  if (name === "car") {
    return (
      <g className="prop prop-car" opacity={object.opacity} transform={`translate(${x} ${y})`}>
        <path d="M-38 4l7-22h46l18 22v18h-71zM-25-18l10-13h23l12 13M-22 4h15m20 0h15" />
        <g className="car-wheel" transform="translate(-22 22)"><circle r="8" /><path d="M0-6V6M-6 0h12" /></g>
        <g className="car-wheel" transform="translate(22 22)"><circle r="8" /><path d="M0-6V6M-6 0h12" /></g>
      </g>
    );
  }
  if (name === "sun") {
    return (
      <g className="prop prop-sun" opacity={object.opacity} transform={`translate(${x} ${y})`}>
        <g className="sun-spin"><circle r="15" /><path d="M0-29v-9M0 29v9M-29 0h-9M29 0h9M-21-21l-7-7M21 21l7 7M21-21l7-7M-21 21l-7 7" /></g>
      </g>
    );
  }
  if (name === "plant") {
    return (
      <g className="prop prop-plant" opacity={object.opacity} transform={`translate(${x} ${y})`}>
        <path d="M-17 6h34l-5 29h-24z" /><path d="M0 6v-28" />
...
function StickFigure({ actor }: { actor: ActorFrame }) {
  ...
  return (
    <g className="actor" data-pose={actor.pose} data-feeling={actor.feeling ?? "none"} data-face={face} style={{ opacity: actor.opacity }}>
      <g className="actor-movement" transform={`translate(${actor.x} ${actor.y}) scale(${actor.scale})`}>
        <g className="actor-body-motion"><g className="actor-react">
          <g className="stick-head">
            <circle cy="-37" r="25" />
            <g className="stick-eyes"><circle className="doodle-fill eye" cx="-9" cy="-41" r="2.5" /><circle className="doodle-fill eye" cx="9" cy="-41" r="2.5" /></g>
            <g className="stick-mouths">
              <circle className="mouth mouth-surprised" cy="-25" r="4" /><path className="mouth mouth-smile" d="M-7-28q7 8 14 0" /><path className="mouth mouth-sleepy" d="M-6-26h12" /><path className="mouth mouth-focused" d="M-5-26h10" /><path className="mouth mouth-worried" d="M-8-24q4-6 8 0t8 0" />
            </g>
            ...
            {showSweat ? <g className="gag gag-sweat"><path d="M22-48c0 8-6 10-6 14 3 1 8-4 8-10 0-4-2-6-2-4Z" /><path d="M30-40c0 6-4 8-4 11" /></g> : null}
          ...
          <g className="stick-arm arm-reach"><path d="M0 5l28 4" /></g>
          <g className="stick-leg leg-left"><path d="M0 58-25 95" /></g>
          <g className="stick-leg leg-right"><path d="M0 58 25 95" /></g>
...
function SceneEffects({ effects, frame }: { effects: EffectFrame[]; frame: FrameState }) {
  return <g className="scene-gags">{effects.map((effect) => {
    const parent = frame.actors.find((actor) => actor.id === effect.parent) ?? frame.objects.find((object) => object.id === effect.parent);
    const x = parent?.x ?? effect.x; const y = parent?.y ?? effect.y;
    if (effect.kind === "papers") return <g key={effect.id} className="gag gag-papers is-on" transform={`translate(${x} ${y - 40})`}><path d="M-18-8h22l-3 18h-22z" /><path d="M8-2h20l4 16h-22z" /><path d="M-6 12h18l-6 14h-16z" /></g>;
    if (effect.kind === "spark") return <g key={effect.id} className="gag gag-spark is-on" transform={`translate(${x - 250} ${y - 165})`}><path d="M250 42l4 10 10 3-8 7 2 11-8-6-8 6 2-11-8-7 10-3z" /><path d="M318 58l3 8 8 2-6 6 1 8-6-4-6 4 1-8-6-6 8-2z" /></g>;
    ...
    if (effect.kind === "exclamation" || effect.kind === "zzz" || effect.kind === "sweat") return null;
...
export function DoodleScene({ scene, t, playing }: { scene: StoryScene; t: number; playing: boolean }) {
  ...
  return (
    <svg className={classes} viewBox="0 0 500 300" role="img" aria-label={`Animated scene: ${scene.caption}`} data-pose={primary?.pose ?? "idle"} data-feeling={primary?.feeling ?? "none"} style={{ "--scene-ms": `${scene.durationMs}ms` } as CSSProperties}>
      <g className="doodle-lines">
        ...
        <SceneEffects effects={frame.effects} frame={frame} />
        {frame.objects.map((object) => <DoodleProp key={object.id} object={object} />)}
        {frame.actors.map((actor) => <StickFigure key={actor.id} actor={actor} />)}
      </g>
    </svg>
  );
}
